...
df_racer = pd.read_csv('steamlit_template/data/racer_stats.csv')
...
df_unp_racer = df_single_racer.unstack().rename_axis(['category', 'row number']).reset_index().drop(columns='row number').rename({0:"strenght"}, axis=1)
st.write(df_unp_racer)
st.bar_chart(df_unp_racer, x='category', y='strenght')
